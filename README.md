...
This creates a new folder in the directory you executed the command in, with whatever name you gave it (i.e. \<cookbook_name\> in the above command).
...
This looks a bit daunting if you don't know ruby, but it's not that hard to understand in plain English.
...
We can test if nginx is running, and whether it is listening on port 80. This is a bit beyond the scope of this walkthrough so we'll leave it here for now.

### :shaved_ice: Kitchen Test :cookie:
One final thing we can do to test our cookbook set up is to run the command:

    kitchen test

in our command line. This will set up a new kitchen/machine, run the provisioning and tests, then destroy the machine. Nice and efficient and a teaser to how we want machines to be operate in our larger systems.
